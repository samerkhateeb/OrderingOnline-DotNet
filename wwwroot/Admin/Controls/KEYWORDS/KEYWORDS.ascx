<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="KEYWORDS.ascx.cs" Inherits="Admin.Admin.Controls.KEYWORDS.KEYWORDS" %>

<asp:Button ID="InsertButton" runat="server" onclick="InsertLinkButton_Click" 
    Text="New Keyword" />
    
<asp:GridView ID="ManagerGridView" runat="server" 
    AutoGenerateColumns="False" DataKeyNames="KEYWORD_ID"
    ...
    <Columns>
        <asp:BoundField DataField="KEYWORD_NAME" HeaderText="Name" />
        <asp:BoundField DataField="KEYWORD_SPID" HeaderText="SubPageID" />
        <asp:BoundField DataField="KEYWORD_PID" HeaderText="PageID" />
        ...
        <asp:TemplateField HeaderText="Edit">
            <ItemTemplate>
                <asp:HyperLink ID="HyperLink1" runat="server" CssClass="GV_EditBtn" 
                    NavigateUrl='<%# String.Format("/admin/default.aspx?id={0}&controlid={1}&status=edit", Eval("KEYWORD_ID"), Request.QueryString["controlid"]) %>' 
                    Text=""></asp:HyperLink>
            </ItemTemplate>
        </asp:TemplateField>
