<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Admin_Rules.ascx.cs" Inherits="Admin.Admin.Controls.Admin_Rules.Admin_Rules" %>

<asp:Button ID="InsertButton" runat="server" onclick="InsertLinkButton_Click" 
    Text="New Admin Rule" />
    
<asp:GridView ID="ManagerGridView" runat="server" 
    AutoGenerateColumns="False" DataKeyNames="Rule_ID"  
    ...
    <Columns>
        ...
        <asp:BoundField DataField="Rule_Name" HeaderText="Name" />
        <asp:BoundField DataField="Rule_Sorting" HeaderText="Sorting" />
        <asp:CheckBoxField DataField="Rule_Status" HeaderText="Status" />
        ...
        <asp:TemplateField HeaderText="Edit">
            <ItemTemplate>
                <asp:HyperLink ID="HyperLink1" runat="server" CssClass="GV_EditBtn" 
                    NavigateUrl='<%# String.Format("/admin/default.aspx?id={0}&controlid={1}&status=edit", Eval("Rule_ID"), Request.QueryString["controlid"]) %>' 
                    Text=""></asp:HyperLink>
            </ItemTemplate>
        </asp:TemplateField>
