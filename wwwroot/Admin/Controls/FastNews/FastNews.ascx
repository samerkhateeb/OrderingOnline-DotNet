<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="FastNews.ascx.cs" Inherits="ControlPanel_CP_Controls_UC_FastNewsManager" %>
...
<asp:GridView ID="ManagerGridView" runat="server" 
    AutoGenerateColumns="False" DataKeyNames="FastNews_ID"
    ...
    <Columns>
        <asp:BoundField DataField="FastNews_Name_Eng" HeaderText="Name" />
        <asp:CheckBoxField DataField="FastNews_Status" HeaderText="Status" />
        <asp:BoundField DataField="FastNews_Sorting" 
            HeaderText="Sorting" />
           ...
           <asp:TemplateField HeaderText="Edit">
            <ItemTemplate>
                <asp:HyperLink ID="HyperLink1" runat="server" CssClass="GV_EditBtn" 
                    NavigateUrl='<%# String.Format("/admin/default.aspx?id={0}&controlid={1}&status=edit", Eval("FastNews_ID"), Request.QueryString["controlid"]) %>' 
                    Text=""></asp:HyperLink>
            </ItemTemplate>
        </asp:TemplateField>
