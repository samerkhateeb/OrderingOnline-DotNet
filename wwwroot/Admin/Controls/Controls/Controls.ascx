<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Controls.ascx.cs" Inherits="Admin.Admin.Controls.Controls.Controls" %>
...
<asp:GridView ID="ManagerGridView" runat="server" 
    AutoGenerateColumns="False" DataKeyNames="Control_ID"  
    ...
    <Columns>
        <asp:BoundField DataField="Control_Name" HeaderText="Name" />
        <asp:CheckBoxField DataField="Control_Status" HeaderText="Status" />
        <asp:BoundField DataField="Control_Sorting" HeaderText="Sorting" />
        ...
        <asp:TemplateField HeaderText="Edit">
            <ItemTemplate>
                <asp:HyperLink ID="HyperLink1" runat="server" CssClass="GV_EditBtn" 
                    NavigateUrl='<%# String.Format("/admin/default.aspx?id={0}&controlid={1}&status=edit", Eval("Control_ID"), Request.QueryString["controlid"]) %>' 
                    Text=""></asp:HyperLink>
            </ItemTemplate>
        </asp:TemplateField>
