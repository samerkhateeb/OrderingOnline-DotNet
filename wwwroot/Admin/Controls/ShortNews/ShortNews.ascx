<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ShortNews.ascx.cs" Inherits="Admin.Admin.Controls.ShortNews.ShortNews" %>
...
<asp:GridView ID="ManagerGridView" runat="server" 
    AutoGenerateColumns="False" DataKeyNames="ShortNews_ID"
    ...
    <Columns>
        <asp:BoundField DataField="ShortNews_Name_Eng" HeaderText="NameEng" />
        <asp:CheckBoxField DataField="ShortNews_Status" HeaderText="Status" />
        <asp:BoundField DataField="ShortNews_Sorting" HeaderText="Sorting" />
        ...
        <asp:TemplateField HeaderText="Edit">
            <ItemTemplate>
                <asp:HyperLink ID="HyperLink1" runat="server" CssClass="GV_EditBtn" 
                    NavigateUrl='<%# String.Format("/admin/default.aspx?id={0}&controlid={1}&status=edit", Eval("ShortNews_ID"), Request.QueryString["controlid"]) %>' 
                    Text=""></asp:HyperLink>
            </ItemTemplate>
        </asp:TemplateField>
